...
one-time mixed-date incident. Named as a candidate for archiving in the
original 2026-08-21 architecture review (§40.6: "already-used, one-time
...
The original executable content is fully preserved in git history:
    git log --follow -- ARCHIVED_stale_write_scripts/fix_mixed_date.py
or the pre-archival commit on `main` (docs/KIRAN_CLEANUP_AUDIT.md §107).
"""

raise RuntimeError(
    "fix_mixed_date.py is archived and neutralized (TR-12, ledger §107). "
