...
Directly ran DELETE against `trade_setups` to fix a one-time Paper/Actual
trade duplication caused by the Excel journal import. Named as a candidate
for archiving in the original 2026-08-21 architecture review (§40.6:
"already-used, one-time scripts with no ongoing purpose") but that step was
never actually executed until now. (A stray filename reference to this
script in the already-obsolete `reorganize.py` "run once" utility is inert
-- not an active import or call.)
...
The original executable content is fully preserved in git history:
    git log --follow -- ARCHIVED_stale_write_scripts/fix_paper_actual.py
or the pre-archival commit on `main` (docs/KIRAN_CLEANUP_AUDIT.md §107).
"""

raise RuntimeError(
    "fix_paper_actual.py is archived and neutralized (TR-12, ledger §107). "
