...
(948 lines, last modified 2026-07-29) sitting in the repo root — capable of
...
this one and 4 others like it). A second, older duplicate
(signal_engine_backup_2c.py) was found and archived alongside it.
...
The original executable content is fully preserved in git history:
    git log --follow -- ARCHIVED_stale_write_scripts/signal_engine_backup_e6.py
or the pre-archival commit on `main` (docs/KIRAN_CLEANUP_AUDIT.md §107).
"""

raise RuntimeError(
    "signal_engine_backup_e6.py is archived and neutralized (TR-12, "
