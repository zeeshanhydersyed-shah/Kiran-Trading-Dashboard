...
| `local_archive_sync.py` | TR-01 shadow-mode Component B (Phase 4). Pulls the authoritative published state (Postgres `current_publication`-promoted, coherent sessions) DOWN into a **separate** `psx_archive.db` — append/replace only, never computes a signal, never opens `psx_data.db`. Read-only PG connection. `--status` prints the sync heartbeat. Not yet scheduled (B-exec is a later signed-off step); ledger §111 |
| `shadow_compare.py` | TR-01 shadow-mode Component C (Phase 5). Per session, diffs the decision-driving digest of every MANDATORY table across Postgres (authoritative) / `psx_data.db` (local compute) / `psx_archive.db` (the pull-sync mirror). CLEAN / DISAGREE (halt + ntfy, counter→0) / INCOMPLETE. Writes `shadow_comparison` in `psx_archive.db`; `--status` shows the clean-session streak. PASSES at streak 10 — evidence for §40.18 criterion 6, not a cutover trigger. Not yet wired into `local_archive_sync.py`'s tail (C-exec is a later signed-off step); ledger §112 |
