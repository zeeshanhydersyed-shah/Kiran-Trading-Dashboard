...
Architecture:
  TradingDeskAgent (orchestrator)
    ├─ PatternAnalyzerAgent    — discovers patterns from your trade history
...
Respond ONLY with valid JSON."""
    ...
    def run(
        ...
        # ── Load learned pattern evidence & reference calibration ─────────────
...
Write in a direct, professional tone. Use Markdown formatting."""
    ...
    def run(self) -> dict:
        ...
        try:
            pattern_result = PatternAnalyzerAgent().run(setups, sector_df)
        except Exception as e:
            logger.error("PatternAnalyzer failed: %s", e)
            pattern_result = {"patterns": [], "key_insight": str(e)}
            errors.append(f"PatternAnalyzer: {e}")
...
TOP PATTERN: {pattern_result.get('top_pattern','?')}
KEY INSIGHT: {pattern_result.get('key_insight','?')}
WHAT TO STOP: {pattern_result.get('what_to_stop_doing','?')}
...
4. 📈 Pattern Insights (what patterns are firing, what to avoid in this regime)
5. ⚡ Performance Check (edge status + one key improvement)
6. ⚠️ What NOT To Do Today (critical in ranging markets — e.g. 'do not chase breakouts')
7. 💡 Action Plan (3 clear bullets)
...
def _build_chat_context() -> str:
    ...
    # ── Active patterns ───────────────────────────────────────────────────────
    try:
        patterns = adb.get_agent_patterns(active_only=True)
        if patterns:
            lines.append(f"\nACTIVE PATTERNS ({len(patterns)}):")
            for p in patterns[:5]:
                wr = f"{p['win_rate_pct']:.0f}%" if p.get("win_rate_pct") else "unknown WR"
                lines.append(f"  [{p.get('confidence','?')} | {wr}] {p['pattern_name']}: {p.get('description','')[:80]}")
    except Exception:
        pass
