...
| `python -m archive.silver_build [--ca-source legacy\|v2]` | **Phase 3, Task 3.2.** Full deterministic rebuild of the Silver layer from the live Bronze: `silver/prices_adjusted/` (CA-adjusted OHLC -- port of `apply_price_adjustments.py`, on DuckDB -- + circuit flags), `silver/sectors/`, `silver/stock_metadata/` (port of `build_stock_metadata.py`'s idempotent upsert). CA events from `corporate_action_suspects_clean.csv` + the frozen baseline's CONFIRMED rows (read-only). `ca_v2_reader` wired behind `--ca-source v2` / `KIRAN_SILVER_CA_SOURCE`, **off by default**. Writes `_silver_build_log.jsonl` + `_silver_parity.json` (vs. the frozen store -- one known residual: DLL, see MEDALLION.md). |
| `python -m archive.gold_build [--window-days N] [--no-parity]` | **Phase 3, Task 3.3.** Rebuild the DuckDB serving store `psx_serving/psx_serving.duckdb` from the live Silver/Bronze -- full replace, staging + atomic swap, deterministic Parquet export per table. **Full DuckDB port** of the screeners: each screener's pure compute core is reused by import, only the I/O is DuckDB. `SCREENERS` registry -- **3.3a** `regime`, **3.3b** `stock_signals` (reuses `stock_signals.py`'s loaders + `_process_trading_dates` verbatim; full universe, 2015 floor, ~8-12 min/run, ~208k rows); 3.3c-f add `sector_signals` / `boring_signals` / etc. `_gold_parity.json` vs the live `psx_data.db` (read-only) verifies the port (`rs_score_20` byte-exact) and surfaces live-side defects (`KIRAN_CLEANUP_AUDIT.md` §118: historical `stock_signals` universe truncation + rank/score inconsistency; `market_regime` gaps). See MEDALLION.md. |
