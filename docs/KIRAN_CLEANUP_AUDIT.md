...
10. <span style="color:#dc2626;">**NEW, MEDIUM — production `market_regime` history has diverged from local, traced to a real `index_prices` data mismatch on at least 2026-07-08 and 2026-07-16.**</span> This is why the dashboard's "days since regime change" dropped from 10 to 5 — confirmed real, not a UI glitch, by querying live Supabase directly. Root mechanical cause (why the two databases' KSE-100 prices differ) not yet found; no fix applied pending that and explicit sign-off. → §16
...
11. <span style="color:#dc2626;">**NEW 2026-07-31**</span> — Root-cause the `market_regime`/`index_prices` divergence between local SQLite and production Postgres for 2026-07-08 and 2026-07-16 (see §16), then decide the correction under this project's production-write sign-off discipline.
...
## 16. <span style="color:#dc2626;">🔴 New finding — `market_regime` history diverges between local SQLite and production Postgres (2026-07-31)</span>

**Trigger:** user noticed the dashboard's "days since regime change" dropped from 10 to 5 between two viewings and asked for the source-level reason.

**Confirmed live against both databases** (read-only, via `SUPABASE_DB_URL` from the local `.env`): local SQLite's `market_regime` shows a clean, single transition into `VOLATILE` on 2026-07-13, holding straight through to 2026-07-30 (`days_since` = 10, matching the recompute logic in `dashboard.py`'s `_get_regime_status()` / `dashboard_pg.py`'s `get_regime_status_pg()`). **Production Postgres has a materially different regime history for the exact same date range**: 07-13 RANGING, 07-14 VOLATILE, 07-15–17 RANGING, 07-20 VOLATILE, 07-21–22 RANGING, 07-23 VOLATILE (holding through 07-30) — five alternations in ten sessions, moving the last transition to 2026-07-23 and `days_since` to 5. **This is what the user is actually seeing on the live Streamlit Cloud app** (which reads Postgres), and the "10 → 5" they observed is real, not a UI glitch or a stale cache.

**Root cause traced one level further, not yet fully closed:** `regime.py`'s classification (`_classify()`) is deterministic given its inputs (EMA-20/50/200, 20-day return, ATR%) — no randomness, no lookahead. Comparing the underlying `index_prices` (KSE-100) between the two databases for the same window found **the raw price data itself differs** on at least two dates: 2026-07-08 (local high/low 185215.56/179504.34 vs. Postgres 188126.67/186189.21 — the two ranges don't even overlap) and 2026-07-16 (close 178123.56 local vs. 175285.78 Postgres, ~1.6% different). Since `_classify()`'s EMA/ATR/return inputs are computed from a rolling 250-row window ending at each date, a price difference on or before a given day can flip its regime classification — which is consistent with what's observed.

**Not yet determined:** the mechanical reason the two `index_prices` tables diverged on those specific dates (scraper timing difference between a local manual run and the scheduled GitHub Actions run, a partial/mid-session capture on one side, or a stale value locked in by the `COALESCE`-style upsert that never got corrected). This needs its own investigation before any fix — per this project's production-write discipline, correcting production's `index_prices` or `market_regime` history requires explicit sign-off regardless, and the fix should target the actual mechanism once found, not just overwrite Postgres with SQLite's version (SQLite isn't verified to be correct either — it just wasn't independently recomputed after the fact).

**Not done:** no code or database change made — this section is a diagnostic finding, not a fix. Flagged as a new, higher-priority backlog item (see §3 revision).
