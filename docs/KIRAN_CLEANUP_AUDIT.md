...
1. <span style="color:#16a34a;">**RESOLVED (in progress) — exposed Groq key.**</span> User confirmed this key was the "Trading Agent Chat" key (a separate "Clinical Psychologist" Groq key is unaffected) and is revoking it directly in the Groq console — no replacement key will be issued; the Agent chat interface falls back to Claude automatically per its documented behavior (`CLAUDE.md`, "LLM cost strategy"). `notes/groq key.txt` has been removed from git tracking and the working tree, and `.gitignore` now excludes `notes/` and `*key*.txt` going forward. **Still open:** the old (soon-to-be-revoked) key value remains readable in prior git commits until history is rewritten — lower urgency now that it's being revoked, but still worth a deliberate yes/no decision. → §7.1
2. <span style="color:#dc2626;">**HIGH — the local working tree has diverged substantially from what git (and therefore Streamlit Cloud) actually has.**</span> Discovered while removing the exposed key: core production files (`main.py`, `database.py`, `scraper.py`, `leaders_scan.py`, `compute_forward_returns.py`, `backfill_setup_log.py`, `apply_price_adjustments.py`, `migrate_to_supabase.py`, `CLAUDE.md`) all have **uncommitted local edits**, and hundreds of files on disk — including production-adjacent ones like `boring_signals.py`, `stealth_rs.py`, `settlement_scraper.py`, `research_db.py` — are **untracked by git entirely**. This means the live Streamlit Cloud app is very likely running different code than what's on this machine right now, and none of this local work is backed up by git history. Needs its own triage pass before anything else in this document touches those files. → §7.3 (new)
...
### 7.1 <span style="color:#16a34a;">🟢 Resolved (in progress) — exposed Groq key</span>
...
- **Done this session:** `notes/groq key.txt` removed from git tracking (`git rm --cached`) and deleted from the working tree; `.gitignore` updated with `notes/`, `*key*.txt`, and `*.key` rules to prevent recurrence. **Not committed** — these two changes sit staged/pending, deliberately kept separate from the large body of unrelated pre-existing uncommitted changes found in §7.3, so they can be committed on their own once you confirm.
- **Still open, lower urgency now:** the old key value remains readable in every prior git commit until history is rewritten (e.g. `git filter-repo` + force-push). Since the key is being revoked rather than rotated-and-reused, the exposure risk from history alone drops substantially — this is now a "nice to have" cleanup, not an active-exposure emergency. Flag if you want it done anyway (it requires a force-push, which needs your explicit go-ahead per this session's own safety rules).
...
### 7.3 <span style="color:#dc2626;">🔴 New this session — local working tree has diverged from git</span>
Discovered as a side effect of removing the exposed key (running `git status` to confirm the key file's tracked state surfaced the wider picture):

- **Core production files carry uncommitted local edits:** `CLAUDE.md`, `main.py`, `database.py`, `database_pg.py`, `scraper.py`, `leaders_scan.py`, `compute_forward_returns.py`, `backfill_setup_log.py`, `apply_price_adjustments.py`, `migrate_to_supabase.py`, plus `breadth_data.csv` and several generated chart/output files. None of this is committed — it exists only on this one machine's disk.
- **Hundreds of files are untracked by git entirely** — not just the loose one-off scripts and backup files already catalogued in §6, but also production-adjacent modules referenced by `CLAUDE.md` itself: `boring_signals.py`, `stealth_rs.py`, `settlement_scraper.py`, `research_db.py`, and both `ZH_research/` and `boring_study/` in their entirety. `.github/workflows/eod-scraper.yml` — one of the six live workflows found in §7.2 — is also untracked.
- **What this means concretely:** the version of Kiran actually running on Streamlit Cloud right now is very likely **not** the same code sitting in this local folder. `CLAUDE.md`'s own documented deploy process ("1. Edit files locally → 2. git add/commit/push → 3. Cloud auto-redeploys") assumes every change gets committed — that assumption has not held for some time, given the size of this gap. It also means none of the uncommitted local work is protected by git history; a disk failure or accidental overwrite right now would lose real, unrecoverable work.
- **This is not something to fix as a side effect of the Groq key cleanup** — the scope and intent behind each modified/untracked file need to be understood before committing anything in bulk (some may be intentional in-progress work, some may be exactly the kind of one-off script this audit already flagged for archiving in §6). Recommend a **dedicated triage session**: `git diff` each modified core file to see what changed and why, decide file-by-file whether untracked files belong in git (production modules) or in `.gitignore` (research/output artifacts), then commit in coherent, reviewable batches rather than one giant catch-all commit.
...
This document is **Phase 0**. Nothing past this point has been started, **except the Groq key response in §7.1** (user is revoking the key directly with Groq; the file has been untracked and `.gitignore` updated, staged but not committed) — that didn't require a cleanup session to justify, since it was a live credential exposure independent of this audit's scope. The git-divergence finding in §7.3 is newly discovered and not yet triaged.
...
- **Phase 3 — Code-quality mechanical pass.** Triage the git-divergence finding first (§7.3 — decide per-file what's committed vs. archived vs. gitignored before touching anything else). Decide on the optional git-history purge for the old Groq key (§7.1). Add lint/format tooling and run once repo-wide. Consolidate `test_*.py` into `tests/` and wire a smoke-test CI workflow (§7.2, §10 Option A).
