...
- **Parity vs live `psx_data.db` (read-only) — `status: clean` (port verified):** `_gold_parity.json`
  ...
  - **`lookback_flag_residual`** — ~10–18 EMA-stack flag flips per sample date on thin names
    ...
    not failed.
  - **Finding — `KIRAN_CLEANUP_AUDIT.md` §118:** live's historical `stock_signals` universe was
    ~290 symbols on every date through 2026-07-31, then +131 **forward-only** on 2026-08-03
    (227→292 organic growth, then the jump). Every pre-2026-08-03 `rs_rank` / `sector_rs_rank` /
    RS-leader signal was over a ~32%-truncated pool. Gold ranks the full Silver universe.
- **Disposition:** do NOT backfill live `stock_signals` (the migration replaces it at cutover).
  Live's rank/score-inconsistency root cause = small open follow-up. The EMA-flag lookback
  residual clears at higher RAM / a streaming-compute refactor (future item). `psx_data.db`
