...
- **DB writes:** **Postgres** — one additive nullable column on `current_publication` (10 → 11 columns, **0 rows affected**, table was and is empty). No other object touched: `pipeline_runs` unchanged (68 rows), public table count 64 → 64. **SQLite** — one new empty table `current_publication` (52 → 53 tables, 0 rows), created with 11 columns incl. `coherence`. `pipeline_runs` unchanged (84 rows), `PRAGMA integrity_check` = ok.
- **Backup first:** SQLite — `backups/psx_data_pre_coherence_20260902.db` (`PRAGMA integrity_check` ok, sha256 `526e6341aff32909b56ef3b541f0cff306c9a6931d6b4e12a90ac4e581e9b6a4`, 882016256 bytes, `.sha256` sidecar written). Postgres (Supabase PITR OFF) — CSV of `current_publication` (0 rows) + a 10-column schema snapshot in `scratch_shadowmode_20260902/`. Rollback: `ALTER TABLE current_publication DROP COLUMN coherence` (PG) / `DROP TABLE current_publication` (SQLite — it holds nothing).
- **Verification:** independent fresh connection on each backend — PG: 11 columns, `coherence text` nullable, 0 rows, 64 public tables, `pipeline_runs` = 68, `ensure_current_publication_pg()` re-run = no-op. SQLite: 11 columns incl. `coherence`, 0 rows, `integrity_check` ok, `pipeline_runs` = 84, `ensure_current_publication_sqlite()` re-run = no-op. Full local test suite 388 passed before this ran.
- **By:** Claude Code, explicit owner sign-off ("go ahead, run steps 1-3"). Full detail: audit ledger §110. OI-13 now closed; PR #67 clear to merge.
