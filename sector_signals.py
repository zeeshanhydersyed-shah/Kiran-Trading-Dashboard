...
indicators, aggregates to sector level, and upserts a row per sector for
every trading date still missing from sector_signals since the last
successful run -- not just the single latest scraped date.

Why a backfill loop: the single-date-only version silently lost any date
whose hook run failed (each run is wrapped in its own try/except in
main.py's cmd_update(), by design, so one bad day can't take down the rest
of the pipeline) -- the next successful run just computed the new latest
date and moved on. That happened for real on 2026-08-07 (see
docs/KIRAN_CLEANUP_AUDIT.md): sector_signals has no row for that date even
though prices_adjusted does. stock_signals.py already backfills a date
range this way; regime.py and this module now match it.
"""
...
def _compute_and_write_sector_signals_for_date_pg(target_date: str) -> int:
    """PG-backed equivalent of the old single-date append_latest_sector_signals().

    Reads all inputs from Supabase, runs the same computation, writes back via
    PG helpers, for exactly one date. Returns the number of sector rows
    written (0 if that date was already populated, or if there was nothing
    to compute). Called in a loop by _append_latest_sector_signals_pg() for
    every date still missing from sector_signals.
...
def _append_latest_sector_signals_pg() -> None:
    """Backfills sector_signals for every trading date missing since the
    last successfully-written row -- see module docstring for why this
    matters instead of only ever computing the single latest date.
    """
    from database_pg import (
        get_prices_adjusted_max_date_pg,
        get_sector_signals_max_date_pg,
        get_prices_adjusted_dates_between_pg,
    )

    latest_date = get_prices_adjusted_max_date_pg()
    if not latest_date:
        logger.warning("Sector signals hook (PG): no data in prices_adjusted.")
        return

    last_written = get_sector_signals_max_date_pg()
    pending_dates = get_prices_adjusted_dates_between_pg(last_written, latest_date)

    if not pending_dates:
        logger.info("Sector signals already up to date at %s (PG).", latest_date)
        return

    done = 0
    try:
        for d in pending_dates:
            _compute_and_write_sector_signals_for_date_pg(d)
            done += 1
    finally:
        logger.info(
            "Sector signals backfill (PG): %d/%d date(s) processed (through %s).",
            done, len(pending_dates), pending_dates[done - 1] if done else "none",
        )


def _compute_and_write_sector_signals_for_date_sqlite(conn: sqlite3.Connection, target_date: str) -> int:
    """SQLite equivalent of _compute_and_write_sector_signals_for_date_pg().

    Computes and writes sector_signals (+ flow enrichment) for exactly one
    date, committing that date's rows before returning. Caller owns the
    connection's lifecycle (opened once, reused across every date in a
    backfill run, closed by the caller) -- this function only uses it.
    Returns the number of sector rows written (0 if already populated).
    """
...
def append_latest_sector_signals() -> None:
    """Backfills sector_signals for every missing trading date since the
    last successfully-written row (PG or SQLite, matching whichever backend
    is active) -- see module docstring for why this matters instead of only
    ever computing the single latest date.
    """
    if _PG_URL:
    ...
    try:
        latest_row = conn.execute("SELECT MAX(date) FROM prices_adjusted").fetchone()
        ...
        last_written_row = conn.execute("SELECT MAX(date) FROM sector_signals").fetchone()
        last_written = last_written_row[0] if last_written_row else None

        if last_written:
            pending_rows = conn.execute(
                "SELECT DISTINCT date FROM prices_adjusted "
                "WHERE date > ? AND date <= ? ORDER BY date",
                (last_written, latest_date),
            ).fetchall()
        else:
            pending_rows = conn.execute(
                "SELECT DISTINCT date FROM prices_adjusted WHERE date <= ? ORDER BY date",
                (latest_date,),
            ).fetchall()
        pending_dates = [r[0] for r in pending_rows]

        if not pending_dates:
            logger.info("Sector signals already up to date at %s.", latest_date)
            return

        done = 0
        try:
            for d in pending_dates:
                _compute_and_write_sector_signals_for_date_sqlite(conn, d)
                done += 1
        finally:
            logger.info(
                "Sector signals backfill: %d/%d date(s) processed (through %s).",
                done, len(pending_dates), pending_dates[done - 1] if done else "none",
            )
    finally:
        conn.close()
